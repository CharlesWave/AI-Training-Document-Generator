...
def generate_training_document(raw_response_prompt, video_path):
    """
    Generates a training document from a video using the Gemini API.
    
    Args:
        raw_response_prompt (str): The prompt for Gemini API
        video_path (str): Path to the video file
        
    Returns:
        str: JSON response from Gemini or dict with processing status
    """
    try:
        ...
        contents = [
            types.Content(
                ...
                parts=[
                    ...
                    types.Part.from_text(text="""Create the training document for this video"""),
                ],
            ),
        ]
        generate_content_config = types.GenerateContentConfig(
            ...
            system_instruction=[
                types.Part.from_text(text=raw_response_prompt),
            ],
        )
...
# Sample Model Resonse
# """{
#  \"Summary\": {
#    \"Context\": \"How to update a table in EDW schema using Git.\",
#    \"Key Topic\": [
#      \"Opening Visual Studio Code and Git repository.\",
#      \"Switching to the develop branch and pulling the latest version.\",
#      \"Creating a new branch from the develop branch.\",
#      \"Locating and modifying the specific code.\",
#      \"Updating the EDW develop files.\",
#      \"Creating a pull request in Azure DevOps.\",
#      \"Copying pull request link and submitting request in Team chat.\",
#      \"Running the pipeline for the modified code\"
#    ],
#    \"Key Takeaway\": \"By following these steps, you will be able to update a table in EDW schema using Git.\"
#  },
#  \"Details\": [
#    \"Open the Visual Studio Code and Git repository.\",
#    \"Switch to the develop branch, as this is the main branch. Then do a pull request so that you can get the latest changes to the branch.\",
#    \"Create a branch from develop, so you can start making changes on your branch. Give it a new name based on Jira ticket number.\",
#    \"Find the code you want to modify. For example, if you are working on the ticket and coupon project, go to the DDL section to find the data conversion table.\",
#    \"In the code that needs to be modified, check for any errors, then save it. The code that was modified will be marked by an 'M' icon.\",
#    \"Navigate to EDW develop, because a table in EDW is being modified.\",
#    \"Input the path of the file into the deploy file, so that the pipeline knows which file to execute.\",
#    \"Make a change in EDW Dev version file, so that the pipeline knows that this is a new version.\",
#    \"Stage the changes, then commit them and push to continue.\",
#    \"Create a pull request on Azure DevOps.\",
#    \"Copy the pull request link and submit to team chat.\"
#  ],
#  \"Timestamps\": [
#    [
#      \"0:15\",
